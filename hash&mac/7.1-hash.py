...
def pad(plaintext):
	return Padding.appendPadding(plaintext,blocksize=Padding.DES_blocksize,
	mode='CMS')
...
if __name__ == "__main__":
	...
	# Hash each char in the message
    for e in message:
        des = DES.new(message[e], DES.MODE_ECB)
        cipher_text = des.encrypt(hash_base_16)
        hash_base_16 = "".join(chr(ord(x) ^ ord(y)) for x ,y in zip(
		hash_base_16, cipher_text))

    print "Padded plaintext: " + padded_plaintext
    print "Encoded hash: " + str(map(''.join, zip(*[iter(base64.b16encode(
	hash_base_16))]*16)))